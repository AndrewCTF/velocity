...
| Version | 1.1 (Draft until merged to `master`). 1.1 points 8.26 and 8.29 at the completed ASVS assessment, adds the documents it produced, and adds RA-21 |
| Adopted | 2026-09-13 |
| Owner | Maintainer |
...
| 5.18 | Access rights | Yes | Write access must be granted, reviewed and removed deliberately | Implemented | Access review recorded 2026-09-13 (`docs/security/isms/operations.md`, Access review record); repeated at each management review | Maintainer |
...
| 8.15 | Logging | Yes | Mutations must be attributable (R23) | Partial | `apps/api/app/audit.py`, `Depends(audit_mutation)` on six routers, `apps/api/tests/test_audit_mutations.py`; log inventory of every sink with access and retention in `docs/security/logging.md`. Gaps: attempt not outcome, best-effort; audit retention only by operator setting and none for the local governed-action log (RA-09) | Maintainer |
...
| 8.20 | Networks security | Yes | SSRF and exposed ports are the main network risks (R10, R11) | Implemented | `apps/api/app/netguard.py`, `apps/api/tests/test_sink_ssrf.py`; loopback binds in `docker-compose.prod.yml` and `docker-compose.yml`; `TRUSTED_PROXIES` in `docker-compose.prod.yml`; inbound and outbound inventory and egress decision in `docs/security/communications.md` (R29) | Maintainer |
| 8.21 | Security of network services | Yes | TLS and edge protection of a deployment | Operator | TLS terminates at the operator's host proxy (`docker-compose.prod.yml` nginx comment); TLS, proxy log and timeout settings in `docs/security/operator-hardening.md` §1; security headers and `limit_req` in `infra/nginx/nginx.prod.conf` | Operator |
...
| 8.24 | Use of cryptography | Yes | Tokens, stored keys and releases rely on cryptography | Partial | Key management policy on NIST SP 800-57 Part 1 principles, per-key permitted use and cryptographic inventory in `docs/security/crypto-and-keys.md` (Fernet BYOK with `MultiFernet` rotation, HMAC-SHA256 JWT, SHA-256 token and evidence hashing, TLS); `API_KEY` and JWT secret minimum length enforced at startup (`apps/api/app/auth.py`, `check_credential_strength`); internal hops plaintext by decision (R28). Gaps: releases unsigned (RA-07); `SUPABASE_JWT_SECRET` also signs the MCP internal token (key-usage separation deviation, `docs/security/crypto-and-keys.md` §1.2) | Maintainer |
...
| 8.26 | Application security requirements | Yes | Requirements must be stated before they can be tested | Partial | Invariants with guard tests in `apps/api/CLAUDE.md`; input validation rules, business limits, timeouts and upload handling in `docs/security/input-and-files.md`; data protection requirements in `docs/security/data-protection.md` §2; communications inventory `docs/security/communications.md`. OWASP ASVS 5.0 Level 2 assessment completed 2026-09-13 in `docs/security/asvs-l2-assessment.md` (RA-11 done); its accepted risks are R26–R31 and its operator requirements are in `docs/security/operator-hardening.md` | Maintainer |
...
| 8.29 | Security testing in development and acceptance | Yes | Fixes need regression tests | Partial | 45 security tests added (`docs/security/gap-analysis-2026-09.md`, after measurements); CI audit and CodeQL; OWASP ASVS 5.0 Level 2 assessment with per-requirement evidence (`docs/security/asvs-l2-assessment.md`, completed 2026-09-13). No DAST or penetration test | Maintainer |
...
| RA-11 | **Done 2026-09-13**: ASVS 5.0 Level 2 assessment completed in `docs/security/asvs-l2-assessment.md`; its accepted risks are R26–R31 in `risk-assessment.md` and its operator items are in `docs/security/operator-hardening.md` | 8.26, 8.29 | R18 | Maintainer | 2026-09-30 |
