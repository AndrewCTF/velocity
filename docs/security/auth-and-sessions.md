...
| WebSocket `?key=` | query string on the upgrade only | same as the credential it carries | Same as the header forms. Redacted from `uvicorn.access` and `uvicorn.error`. Ignored on plain HTTP. | `require_ws_key` |
