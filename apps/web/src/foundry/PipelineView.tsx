...
// Pipeline — the lineage DAG (the signature Foundry artifact: datasets as
// rounded nodes, transforms as diamonds, edges as bezier curves laid out in
// topological layers left→right; nodes tint by kind and glow when stale) plus
// the transform editor (step DSL, form-per-step + JSON fallback + preview).
      ...
      <div className="flex-1 grid grid-cols-2 gap-1.5">
        {step.type === 'select' && (
          <input
            ...
            value={(step.columns as string[] | undefined)?.join(',') ?? ''}
        ...
        {step.type === 'rename' && (
          <input
            ...
            value={Object.entries((step.map as Record<string, string>) ?? {}).map(([k, v]) => `${k}:${v}`).join(', ')}
            onChange={(e) => {
        ...
        {step.type === 'filter' && (
          <input
            ...
            placeholder="expr, e.g. speed > 10 and country == 'DE'"
            value={(step.expr as string | undefined) ?? ''}
...
function TransformEditor({ editing, onDone }: { editing: Transform | null; onDone: () => void }): JSX.Element {
  ...
  const previewTransform = useFoundry((s) => s.previewTransform);
  ...
  const [preview, setPreview] = useState<{ schema: { name: string; type: string }[]; rows: Array<Record<string, unknown>>; quarantined?: number } | null>(null);
  ...
  const save = async (): Promise<void> => {
    ...
    if (editing) await updateTransform(editing.id, body);
    else await createTransform(body);
    onDone();
  };
  const runPreview = async (): Promise<void> => {
    if (!editing) return;
    setPreview(await previewTransform(editing.id, 20));
  };
    ...
    <div className="rounded-md border border-line-2 bg-bg-1 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="text-[11px] font-semibold tracking-[0.09em] uppercase text-txt-2">
          {editing ? `Edit transform · ${editing.name}` : 'New transform'}
        </div>
        <button type="button" onClick={onDone} className="text-txt-3 hover:text-txt-0 text-[12px]" aria-label="Close editor">✕</button>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <Btn onClick={() => void runPreview()} disabled={!editing} {...(editing ? {} : { title: 'Save the transform first to preview' })}>Preview</Btn>
        {preview && (preview.quarantined ?? 0) > 0 && <Badge tone="warn">{preview.quarantined} quarantined</Badge>}
      </div>

      {preview && (
        <div className="overflow-auto rounded-sm border border-line max-h-52">
...
export function PipelineView(): JSX.Element {
  ...
  useEffect(() => {
    ...
    void loadTransforms();
    void loadDatasets();
  }, [loadLineage, loadTransforms, loadDatasets]);
  ...
  return (
    <div className="p-5 space-y-4">
      ...
      <div className="flex gap-4">
        <div className="flex-1 min-w-0 overflow-auto rounded-md border border-line-2 bg-bg-1" data-testid="lineage-dag" style={{ backgroundImage: 'radial-gradient(var(--line) 1px, transparent 1px)', backgroundSize: '18px 18px' }}>
          <svg width={Math.max(width, 300)} height={Math.max(height, 140)}>
            {(lineage?.edges ?? []).map((e, i) => {
              ...
              const mx = (x1 + x2) / 2;
              return <path key={i} d={`M ${x1} ${y1} C ${mx} ${y1}, ${mx} ${y2}, ${x2} ${y2}`} fill="none" stroke="var(--line-2)" strokeWidth={1.5} />;
            })}
            {laid.map((n) => {
              const { fill, stroke } = nodeColors(n);
              const sel = selected?.id === n.id;
              return (
                <g key={n.id} transform={`translate(${n.x},${n.y})`} onClick={() => setSelected(n)} className="cursor-pointer" data-testid={`lineage-node-${n.id}`}>
                  {n.type === 'dataset' ? (
                    <rect width={NODE_W} height={NODE_H} rx={5} fill={fill} stroke={sel ? 'var(--mag)' : stroke} strokeWidth={sel ? 2.5 : 1.25} />
                  ) : (
                    <polygon points={`${NODE_W / 2},2 ${NODE_W - 4},${NODE_H / 2} ${NODE_W / 2},${NODE_H - 2} 4,${NODE_H / 2}`} fill={fill} stroke={sel ? 'var(--mag)' : stroke} strokeWidth={sel ? 2.5 : 1.25} />
                  )}
        ...
        <div className="w-[264px] shrink-0 rounded-md border border-line-2 bg-bg-1 p-3">
            ...
            <div className="space-y-2.5">
              ...
              {selected.type === 'transform' && (
                <div className="flex flex-col gap-1.5 pt-1">
                  <Btn tone="accent" size="sm" onClick={() => void buildTransform(selected.id)}>Build</Btn>
                  <Btn size="sm" onClick={() => { const t = transforms.find((x) => x.id === selected.id); if (t) { setEditingTransform(t); setEditorOpen(true); } }}>Edit</Btn>
                  <Btn size="sm" onClick={() => { if (window.confirm('Delete this transform?')) void deleteTransform(selected.id); }}>Delete</Btn>
                </div>
              )}
            </div>
          )}
