...
"""
import asyncio, math, urllib.parse, urllib.request, json
import numpy as np
...
async def main():
    print("OSM footprints...", flush=True); blds = overpass(); print("buildings:", len(blds), flush=True)
    ...
    def s2col(lon, lat):
        c = int((lon - W) / (E - W) * (sw - 1)); r = int((N - lat) / (N - S) * (sh - 1))
        c = min(max(c, 0), sw - 1); r = min(max(r, 0), sh - 1)
        return s2[r, c] / 255.0

    def dmg(lon, lat):
        x, y = cdse.lonlat_to_3857(lon, lat); minx, miny, maxx, maxy = cb
        col = int((x - minx) / (maxx - minx) * cw); row = int((maxy - y) / (maxy - miny) * ch)
        return 0 <= row < change.shape[0] and 0 <= col < change.shape[1] and change[row, col] < -0.35

    def area(ring):
        p = np.array([(mx(a), my(b)) for a, b in ring])
        return 0.5 * abs(np.dot(p[:-1, 0], p[1:, 1]) - np.dot(p[1:, 0], p[:-1, 1])) if len(p) > 2 else 0
    ...
    gn = 220
    gx = np.linspace(mx(W), mx(E), gn); gy = np.linspace(my(S), my(N), gn)
    GX, GY = np.meshgrid(gx, gy)
    s2g = np.asarray(Image.fromarray(s2).resize((gn, gn)))[::-1] / 255.0  # flip rows: y up
    ax.plot_surface(GX, GY, np.zeros_like(GX), facecolors=s2g, shade=False, rstride=1, cstride=1, antialiased=False)

    faces, fcolors = [], []; ndmg = 0
    for ring, h in blds:
        pts = [(mx(a), my(b)) for a, b in ring]
        clon = sum(a for a, _ in ring) / len(ring); clat = sum(b for _, b in ring) / len(ring)
        d = dmg(clon, clat); ndmg += int(d)
        ...
        faces.append([(x, y, hh) for x, y in pts]); fcolors.append(roof)
        for i in range(len(pts) - 1):
            x0, y0 = pts[i]; x1, y1 = pts[i + 1]
            faces.append([(x0, y0, 0), (x1, y1, 0), (x1, y1, hh), (x0, y0, hh)]); fcolors.append(wall)
    ax.add_collection3d(Poly3DCollection(faces, facecolors=fcolors, edgecolors=(0, 0, 0, 0.12), linewidths=0.08))
    ax.set_xlim(mx(W), mx(E)); ax.set_ylim(my(S), my(N)); ax.set_zlim(0, 90)
    ax.set_box_aspect((1, 1, 0.4)); ax.view_init(elev=28, azim=-60); ax.set_axis_off()
    ax.set_title(f"Beirut Dahieh — LOD1 + real Sentinel-2 texture | red = SAR collapse ({ndmg})", fontsize=12)
