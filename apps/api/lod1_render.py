...
"""
import asyncio, math, urllib.parse, urllib.request, json, sys
import numpy as np
...
async def main():
    ...
    change = res["_change"]; cb = res["_bbox"]; cw, ch = res["_size"]
    ...
    for ring, h in blds:
        ...
        # roof
        faces.append([(x, y, hh) for x, y in pts]); fcolors.append(top or "#c9bfae")
        # walls
    ...
    xs = [mx(W), mx(E)]; ys = [my(S), my(N)]
    ax.set_xlim(xs); ax.set_ylim(ys); ax.set_zlim(0, 80)
    ax.set_box_aspect((1, 1, 0.35)); ax.view_init(elev=32, azim=-55)
    ax.set_axis_off()
    ax.set_title(f"LOD1 3D — Beirut Dahieh ({len(blds)} OSM footprints) | red = SAR collapse candidate ({ndmg})", fontsize=11)
