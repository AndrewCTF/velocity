...
# Link relations. Not an enum (the registry must hold an edge an analyst or an
# agent invents), and no longer a second hand-kept list: the verbs, both of each
# verb's names, and the kinds its endpoints are expected to be all live in
# ``intel/ontology_schema.py``. Keeping membership derived is why the drift that
# module's header documents cannot happen again.
KNOWN_RELS: frozenset[str] = frozenset(REL_TYPES)
